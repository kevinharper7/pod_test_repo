...
print(type(books)) # book is a 'list'
...
# Return: Not needed for this function
def check_out(book):
    books.remove(book)  # .remove works for lists
...
search_by_name('Down these mean streets')
print()
...
print(type(books_with_details)) # it is a list, due to use of hard brackets, dictionaries use curly brackets
# 2.1
...
def search_by_author(author):
    ...
    for book in books_with_details:
        # books author matches the search term
            if book['author'] == author:
                # if author is on the list, add it to new authors_book list without removing it from older list
                author_books.append(book['title'])
    return author_books

         
search_by_author('Ash')
