...
result = sp.search(name, type='artist')  # fill this in
...
artist_id = result['artists']['items'][0]['id']  # fill this in
print(artist_id)
print("Question 4")
...
chosen_artists = ['Nicki Minaj', 'Megan Thee Stallion',
'Rihanna', 'Dua Lipa', 'BTS']
popularities = []
for artist in chosen_artists:
# spotify search the artist
search_result = sp.search(artist, type='artist')
# get the artist unique id
artist_id = search_result['artists']['items'][0]['id']
# get the artist info from the artist id
artist_info = sp.artist(artist_id)
# Set up a dictionary for that artist with name, popularity, and followers
artist_dict = {}
...
# Append that artist's dictionary to the list of all artists
...
print(sim['artists'][0]['name'])
print(sim['artists'][0]['popularity'])