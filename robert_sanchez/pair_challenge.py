...
def create_account():
    ...
    password = input('Creat a secure password: ')
...
def withdraw(account, amount):
    if amount <= account['balance']:
        ...
        print(f'Got no money dude! You only have ${account['balance']} available') 

deposit_money(random_account, 172)
...
def add_tweets(user_acct):
    tweet = input("What's happening?: ")
    user_acct["tweets"].append(tweet)
    print(tweet)
while len(ash["tweets"]) < 5:
    add_tweets(ash)
​
​
def print_tweets(user_acct):
    for tweet in user_acct["tweets"]:
